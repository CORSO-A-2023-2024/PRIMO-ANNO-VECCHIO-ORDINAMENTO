...
public class Settembre2017 {
	 ...
	 */
	 ...
	 static boolean ex1(int[][] a, int[][]b){
		...
		if(a != null & b != null && a.length == b.length){
			ogni = !(b.length == 0);
			for(int i = 0; i < b.length && ogni; i++){
				boolean ogniRiga = !(b[i].length == 0 || b[i] == null);
				for(int j = 0; j < a[i].length && ogniRiga && b[i].length == a[i].length; j++){
					ogniRiga = b[i][j] >= a[i][j];
				}
				ogni = ogniRiga;
			}
		}else ogni = false;
	 ...
	 * DOVE SCRIVERE ex2: immediatamente al di sotto di questo commento.  
	  */
	  ...
	  static void scambia(int[] a, int l, int r){
		  if(l == r){
			  int s = a[l];
			  if(l+1 < a.length && l % 2 == 0){
				  a[l] = a[l+1];
				  a[l+1] = s;
			  }
		  }
